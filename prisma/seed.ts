  ...
  for (let weekIndex = 0; weekIndex < weekStarts.length; weekIndex++) {
    ...
    await prisma.weeklyExtras.create({
      data: {
        ...
        twitterFollowerCount: isCurrentWeek ? null : 2000 + weekIndex * rand(seed, 15, 40),
        twitterEngagement: isCurrentWeek ? null : rand(seed + 1, 50, 600),
        twitterMetricsNaReason: isCurrentWeek ? "N/A — not pulled yet this week" : null,
        // Impressions are manual-only forever (the scraper exposes no view count) —
        // seeded as an explicit N/A even for past weeks, not a real pulled number.
        twitterImpressions: null,
        twitterImpressionsNaReason: "N/A — not exposed by the scraper, native Twitter Analytics required",
        twitterMetricsPulledAt: isCurrentWeek ? null : new Date(),
        ...
        discordActiveMembersNaReason: isCurrentWeek ? "N/A — manual weekly count not done yet" : null,
        discordTotalMembers: isCurrentWeek ? null : 400 + weekIndex * 15,
        discordTotalMembersNaReason: isCurrentWeek ? "N/A — not pulled yet this week" : null,
        discordNewMembers: isCurrentWeek ? null : rand(seed + 4, 2, 25),
        discordNewMembersNaReason: isCurrentWeek ? "N/A — not pulled yet this week" : null,
        discordNewMembersPulledAt: isCurrentWeek ? null : new Date(),
      },
    });
